...
#inputs
file = "Documents/surfacic and lineic/[500pax]10.events.xml" #this is the event file generated by MATsim
mapfile = "Documents/surfacic and lineic/network_rouen_moyen.xml" #this is the network file
pollutant = "NOX" #the pollutant studied

#outputs
emissions_file = "Documents/surfacic and lineic/total_emissions.csv" #gives the emission for each hour for each link in the ARIA format
map_csv_file = "Documents/surfacic and lineic/map.csv" #gets the network in the simplest format
output_file = "Documents/surfacic and lineic/surfacic_emissions.csv" #gives the emission for each hour for each rectangle in the ARIA format
output_file_center = "Documents/surfacic and lineic/lineic_emissions.csv" #gives the emission for each hour for each rectangle in the ARIA format
surface_network = "Documents/surfacic and lineic/srf_network.csv" #a division of the zone in rectangles compatible with ARIA
lineic_network = "Documents/surfacic and lineic/lineic_network.csv" #gets the network in a form compatible with ARIA for the chronological emissions
...
def emissions_to_dict(pollutant):
    ...
    for event in events :
        ...
        if event_type=="coldEmissionEvent" or event_type=="warmEmissionEvent" :
            ...
            pol = float(event[pollutant])
            time = int(float(event["time"]))//3600
            if link in emissions[str(time)]:
                emissions[str(time)][matsim_id_to_int(link)]+=pol/1000 #g into kg
            else :
                emissions[str(time)][matsim_id_to_int(link)]=pol/1000 #g into kg
...
def emissions_to_csv():
    ...
    csv.write("SRCEID;DATEDEB;DATEFIN;Q_"+pollutant+"\n")
    for hour in range(24):
...
def create_surfacic_emissions(hour):
    ...
    #Density matrix
    surfacic_emissions=np.zeros((resolution,resolution))
        ...
        if not ( (x_min_box <= x1 <= x_max_box and y_min_box <= y1 <= y_max_box) or (x_min_box <= x2 <= x_max_box and y_min_box <= y2 <= y_max_box)):
            if link in emissions[hour]: #if there are emissions :
                for i in range(division):
                    ...
                    surfacic_emissions[int((x-x_min)//dx),int((y-y_min)//dy)] += emissions[hour][link]/division
...
def create_csv_surfacic():
    ...
    f.write("SRCEID;DATEDEB;DATEFIN;Q_"+pollutant+"\n")
...
def create_surface_network():
    ...
    new = []
    new.append("SRCEID;IDADM1;IDADM2;NAMADM;NAMGP1;NAMGP2;NAMGP3;IDCLA1;IDCLA2;IDCLA3;IDCLA4;IDCLA5;IDPROJ;SHIFTX;ORIGX;ORIGY;IDZH;X1;Y1;X2;Y2;X3;Y3;X4;Y4;DZ;ENVOL;SPEEDX;SPEEDY;SPEEDZ;IDUNIT_"+pollutant+";Q_"+pollutant)
    ...
    for i in range(resolution):
        for j in range(resolution):
            ...
            line += "0;0;0;0;0;3;0" #the 3 corresponds to units, here in kg/hour
    ...
    for hour in range(24):
        for link in emissions[str(hour)]:
            ...
            if (x_min_box <= float(x1) <= x_max_box and y_min_box <= float(y1) <= y_max_box) or (x_min_box <= float(x2) <= x_max_box and y_min_box <= float(y2) <= y_max_box):
                if emissions[str(hour)][link]>10**-4:
                    f.write(ids[link]+";"+date+time_format(hour*3600)+";"+date+time_format((hour+1)*3600)+";"+str(round(emissions[str(hour)][link],6))+"\n")
    f.close()
    return
...
def create_lineic_network():
    ...
    new = []
    new.append("SRCEID;IDADM1;IDADM2;NAMADM;NAMGP1;NAMGP2;NAMGP3;IDCLA1;IDCLA2;IDCLA3;IDCLA4;IDCLA5;IDPROJ;SHIFTX;ORIGX;ORIGY;IDZH;X1;Y1;ZH1;X2;Y2;ZH2;SLOPE;WIDTH;IDCANYON;SPEEDX;SPEEDY;SPEEDZ;IDUNIT_"+pollutant+";Q_"+pollutant)
    ...
    for link in link_dict:
        ...
        if (x_min_box <= float(x1) <= x_max_box and y_min_box <= float(y1) <= y_max_box) or (x_min_box <= float(x2) <= x_max_box and y_min_box <= float(y2) <= y_max_box):
            ...
            line += ";0;-999;-999;-1;0;0;0;3;0" #the 3 corresponds to units, here in kg/hour
